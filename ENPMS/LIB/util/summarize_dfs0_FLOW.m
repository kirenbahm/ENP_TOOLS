...
for i = 1:n
   ...
   C = strsplit(F,'_');                                 % breaks apart 'F' by the '_' delimeter and creates the 'C' array
   STATION_NAME = C{1};                                 % populates the STATION_NAME variable with position '1' of array 'C'
   TYPE = 'Discharge';                                  % new variable written as script 'Discharge'
   ...
   try
      ...
      X = MAP_STATIONS(char(STATION_NAME)).X;           % sets the Longitude value from 'MAP_STATIONS' container for each STATION iteration
      Y = MAP_STATIONS(char(STATION_NAME)).Y;           % sets the Latitude value from 'MAP_STATIONS' container for each STATION iteration
      fprintf('... reading %d/%d: %s: %s: with N: %d: Records: %f : %f \n', i, n, char(STATION_NAME), char(TYPE),N_OBS, X, Y);              % Prints to the screen a "working" message notifying user: reading 'iteration #' of 'total iterations' for 'station' : 'data type' with value '0' at 'Lat', 'Long'
      ...
      fprintf('... notfound %d/%d: %s: %s: with N: %d:Records\n', i, n, char(STATION_NAME), char(TYPE),N_OBS);      % Returns a message if there was an error with any of the previous steps under the "Try"
   end
end
