...
if exist(INI.fileM11WM, 'file')
    ...
    %this is not needed because it sets zeros to NaN, not correct for
    %discharges
else
...
DATA = determineChainageM11(INI,DATA);
...
% the above not needed, moved to determineChainageM11

fi = 0;
fn = 0;
ii = 0;

% The loop below has been modified to improve code efficiency and eliminate
% errors.
% for i=1:SZ(2)
...
for K = mapM11chain.keys
   
    DATANEAR = dataNearChainageM11(INI,char(K),DATA);
    NAME = mapM11chain(char(K));
    if isempty(DATANEAR)
        fn = fn + 1;
        XNFOUND{fn} = K;
        fprintf(':%i: WARNING: Requested Station not found \t%s:: \t NOT found::\n',fn,char(NAME));
        continue
    end
    
    fi = fi + 1;
    fprintf('::%d. M11 Station %s requested as %s mapped to: %s\n',fi,char(NAME), char(K), char(DATANEAR.M11CHAIN));
    STATION = INI.mapCompSelected(char(NAME));
    STATION.M11NAME = STATION.STATION_NAME;
    STATION.M11UNIT = DATANEAR.UNIT;
    STATION.M11TYPE = DATANEAR.TYPE;
    STATION.M11T = DATANEAR.T;
    STATION.M11V = DATANEAR.V;
    STATION.TIMEVECTOR = DATANEAR.T;
    STATION.DCOMPUTED = STATION.M11V;
    if strcmp(STATION.M11UNIT,'m')
        STATION.DCOMPUTED = STATION.M11V/0.3048;
        STATION.UNIT = 'ft';
...
% this is needed to eliminate Microsoft Excel limitations to sheet names <
% 30 characters, the code truncates anything above 30, which may cause
% overwriting existing sheet in file LOG.xlsx, not critical
if length(XLSH)> 30
