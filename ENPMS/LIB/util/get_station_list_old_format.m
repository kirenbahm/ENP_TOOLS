function [data] = get_station_list_old_format(infile)
% This reads the selected_station_list text file in the OLD format.
% The old format included 'subsection' headers and an 'END'
% line, used in LaTeX.
% A new system for subsection (areas) was integrated into the scripts 11/2017
...
while  (~feof(fid))
    ...
    elseif ~isempty(regexp(tmp,'END', 'once'))
        break;
    elseif ~isempty(regexp(tmp,'^#', 'once')) % throw out comment lines
        trash=1;
